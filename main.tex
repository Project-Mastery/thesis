...
One of the best tools from this area of study comes in the form of simulation models \cite{8718399}. Computer simulations allow us to check how pedestrians will behave in a certain environment without having to spend time and resources on the construction and evaluations of scale models in the real world. This reduces costs and promotes fast iterative designs that may better align with the requirements of spaces. It also bypasses ethical considerations that arise when attempting to replicate crowd crushes and other deadly situations for research. Study about these can happen via the records of past events, but these are limited as these rarely happen under circumstances where data can be effectively collected. For these scenarios, simulations are some of the only ways to gather more profound insight.

There are many  models that have been made for simulating the flow of pedestrians in an environment \cite{kouskoulisSystematicReviewPedestrian2017}. However, designing and implementing models capable of reproducing phenomena seen in the real world is complicated. Pedestrian dynamics is an inherently interdisciplinary science due to its object of study \cite{kleinmeierVadereOpenSourceSimulation2019}. It relies on concepts from areas as diverse as physics, engineering, psychology, computer science, and sociology. Simpler models may take into account only the physical part of crowd movement and ignore all else, while others deal with the effects of having people with different ages \cite{gorriniAgeGroupdrivenPedestrian2016}, disabilities \cite{fuExperimentalStudyBidirectional2022} or even states of mind such as calm and panic \cite{Alrashed_Shamma_2020}.
...
The work required to complete this dissertation was divided into the tasks below. Tasks 1-4 were done sequentially, while task 5 was done in parallel with all the others.

\begin{enumerate}
  \item Study the current state of pedestrian simulations with a focus on the models used
  \item Research 
  \begin{enumerate}
    \item Reserch of existing simulation frameworks used in the area of pedestrian simulations  
    \item Research of current tools and frameworks used for implementing the ECS architecture
  \end{enumerate}
  \item Proposal and implementation of novel simulation framework
  \item Perform a comparative analysis of the new framework and existing ones in terms of performance and flexibility
  \item Writing and editing the final text of the dissertation
\end{enumerate}
...
Before creating a simulator itself or working on the comparison with available alternatives, it was first necessary to understand pedestrian simulations. This chapter contains a distilled form of the insights gathered during the research process.
