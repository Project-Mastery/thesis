...
Finally, \gls{ecsmos} does not currently have a proper external file format for storing scenarios definitions. Therefore, defining a scenario is currently done directly via code. Such as system is possible, but \gls{ecs} creates some additional complexities when it comes to this, most notably in the definition of system dependencies. While this is possible to do, it was not considered inside the scope of the current work.
...
% During the development of the qualitative scenarios, it was noticed that small variations on the topology (in some situations only on the order of millimeters) may cause large changes to the total simulation time, specially in more complicated scenarios. This behavior was found on all studied simulators and there is no single cause for this. On some cases the main factor seams to be the pathfinding algorithm, which may make drastic changes to the paths of pedestrians over small changes on topology. On others cases, it is harder to find a specific culprit, as there does not seem to be any appreciable difference in pedestrian movement, but their flow eventually slows down due to the formation of jams in some region. 

% This is why most simulators use some level of randomness.


% For the purposes of a user, even large changes to pedestrian path selection are not concerning in most cases because the point of such simulation is the average behavior over multiple run, not the behavior of specific pedestrians or even the paths selected during a single run. All of these simulators also allow the use of randomness in each run. Even considering the cascading nature of these simulations where small changes to one simulation step cause large changes to the following, the trend lines remain consistent. Therefore, this has a minor effect on the quality of the output.

% However, they can have much more noticeable effects when only the performance matters. In this case, where it is desirable to create test environments as replicable as possible, accidentally hitting one of those poor performance sources

% is 
% As a result of this, a simple comparison of the performance results may have some limitations, as very similar environments can 

% Due to the fact that all evaluated simulators exhibit this behavior and that the analysis of such minutia is not withing the scope of this work, it was decided to use simple scenarios where the behaviors of each simulator were well understood.
